module.exports = function() {
  ...
  var orbitDispatch = that.dispatch('tick');
  var tickInterval;
  var orbitalRings = [];
  ...
  _orbitLayout.children = function(_function) {
      ...
      //Probably should use that.functor to turn a string into an object key
    ...
    function traverseNestedData(_node) {
      ...
      if(childrenAccessor(_node)) {
        ...
        if (fixedOrbitArray[_currentRing-1]) {
          ...
          var thisPie = that.pie().value(function(d) {return childrenAccessor(d) ? 4 : 1});
