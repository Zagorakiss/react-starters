...
import {ImageCacher} from 'components/ImageCacher';

class OrbitsCanvas extends React.PureComponent {
    ...
    constructor(props) {
        ...
        this.marketcapColors = [
            {pct: 0.0, color: {r: 0x20, g: 0xD3, b: 0x75}},
            {pct: 0.5, color: {r: 0x1E, g: 0x7B, b: 0x96}},
            {pct: 1.0, color: {r: 0x40, g: 0x11, b: 0x8E}}
        ];
        // Если капитализация проекта больше или равна это параметру, то показываем логотип проекта вместо точки с заливкой
        this.majorMarketcap = 3000;
        // this.majorMarketcap = 180000;
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (!_.isEqual(prevProps.data, this.props.data)) {
            this.setState(this.getStateFromProps(this.props, this.state), this.drawCanvas);
        }
    ...
    getStateFromProps(props, state, isInitial = false) {
        ...
        if (isInitial) {
            Object.keys(props.data).forEach((year, i, years) => {
                Object.values(props.data[year]).forEach((project, j, projects) => {
            ...
            Object.keys(preparedData.data).forEach((year) => {
                let isHiddenYear = true;
                Object.keys(preparedData.data[year]).forEach((key) => {
    ...
    drawCanvas() {
        ...
        // Получаем контекст canvas, чтобы передать его в методы, которые будут отрисовывать графику
        const ctx = this.element.getContext('2d');
        // Очищаем canvas от предыдущего состояния
        ctx.clearRect(0, 0, this.element.width, this.element.height);
        this.center = {
            x: this.element.width / 2,
            y: this.element.height / 2
        };
        // Вызываем метод для отрисовки орбит
        this.drawYears(ctx);
    ...
    drawYears(ctx) {
        ...
        // Проходимся по объекту из state, где проекты сгруппированы по годам
        Object
            .keys(newState.data)
            .forEach((year, i, years) => {
                const {zoomLevel} = newState;
                // Начинам отрисовку орбиты
                ctx.beginPath();
                // Меняем цвет орбиты, если она первая или последняя по счеты. У пустых орбит заливка прозрачная
                if (newState.hiddenYears.indexOf(year) !== -1) {
                    ctx.strokeStyle = '#45464b00';
                } else if (i === 0 || i === years.length - 1) {
                    ctx.strokeStyle = '#7d7bf7';
                } else {
                    ctx.strokeStyle = '#45464b';
                }
                ctx.lineWidth = 1;
                ctx.setLineDash([5, 5]);
                // Радиус окружности высчитывается из сложения радиуса центрального круга и размера промежутка умноженного на индекс
                const orbitRadius = (this.radius + this.gap * i) * zoomLevel.actual;
                ctx.arc(this.center.x + newState.dragOffset.actual.x, this.center.y + newState.dragOffset.actual.y, orbitRadius, 0, 2 * Math.PI);
                ctx.stroke();
                ctx.closePath();
                // Создаем константу, в которой хранятся необходимые для отрисовки проектов данные из этой итерации
                const cycle = {
                    year,
                    orbitRadius,
                    entropy: Math.random() * Math.PI
                };
                // Во время выполнения метода для отрисовки проектов в state будут внесены изменения, которые
                // будут применены после завершения всех итераций по объекту state.data
                newState = this.drawProjects(ctx, newState.data[year], cycle, newState);
    ...
    drawProjects(ctx, projects, cycle, newState) {

        const {orbitRadius, entropy} = cycle;
        const {zoomLevel} = newState;

        Object.values(projects).forEach((project, i, arr) => {
            ...
            const isMajor = project.marketcap >= this.majorMarketcap;
            ...
            switch (newState.event) {
                case 0:
                    x = this.center.x + newState.dragOffset.actual.x + (orbitRadius * zoomLevel.actual * Math.cos(angle));
                    y = this.center.y + newState.dragOffset.actual.y + (orbitRadius * zoomLevel.actual * Math.sin(angle));
                    break;
                case 1:
                    x = (project.coords.x - this.center.x - newState.dragOffset.prev.x) / newState.zoomLevel.prev * newState.zoomLevel.actual + this.center.x + newState.dragOffset.actual.x;
                    y = (project.coords.y - this.center.y - newState.dragOffset.prev.y) / newState.zoomLevel.prev * newState.zoomLevel.actual + this.center.y + newState.dragOffset.actual.y;
                    break;
                case 2:
                case 3:
                    x = (project.coords.x - this.center.x - newState.dragOffset.prev.x) + this.center.x + newState.dragOffset.actual.x;
                    y = (project.coords.y - this.center.y - newState.dragOffset.prev.y) + this.center.y + newState.dragOffset.actual.y;
                    break;
                default:
                    break;
            }

            // if (typeof project.coords !== 'undefined') {
            //
            //     x = (project.coords.x - this.center.x - newState.dragOffset.prev.x) / (newState.dragData ? 1 : newState.zoomLevel.prev) * (newState.dragData ? 1 : newState.zoomLevel.actual) + this.center.x + newState.dragOffset.actual.x;
            //     y = (project.coords.y - this.center.y - newState.dragOffset.prev.y) / (newState.dragData ? 1 : newState.zoomLevel.prev) * (newState.dragData ? 1 : newState.zoomLevel.actual) + this.center.y + newState.dragOffset.actual.y;
            //
            // } else {
            //
            //     // Если его там нет, записываем в нее координаты центра точки проекта и координаты крайних точек,
            //     // с помощью метода shiftProjects немного сдвигаем проекты с орбит (или не сдвигаем)
            //     // x = this.center.x + (orbitRadius * Math.cos(angle)) + this.shiftProjects();
            //     // y = this.center.y + (orbitRadius * Math.sin(angle)) + this.shiftProjects();
            //
            //     // Закоментировано потому что сейчас в хэш-таблицу координаты надо записывать в любом случае из-за зума
            //
            // }

            newState.data[cycle.year][project.key] = {
                ...project,
                bounds: [
            ...
            if (project.isActive) {
                if (isMajor) {
                    ...
                    ctx.beginPath();
                    ctx.filter = 'blur(6px)';
                    ctx.fillStyle = this.getColorForPercentage(1.0);
                    ctx.arc(x, y, projectRadius, 0, 2 * Math.PI);
                    ctx.fill();
                    ctx.closePath();
                    ctx.filter = 'blur(0px)';
                    const img = this.cache.getImageByName(project.key);
                    ctx.drawImage(img, x - projectRadius + 2, y - projectRadius + 2, imgSide, imgSide);
                    ...
                    ctx.fillStyle = this.getColorForPercentage(project.marketcap / this.majorMarketcap);
                    ctx.arc(x, y, projectRadius, 0, 2 * Math.PI);
                    ctx.fill();
                    ctx.closePath();
    ...
    getColorForPercentage(pct) {
        ...
        const toHex = (key) => {
            ...
            return ('0' + parseInt(shade, 10).toString(16)).slice(-2);
        };
    ...
    handleZoom(level) {
        this.setState((prevState) => {
            ...
            const isAlright = newActualZoomLevel >= 0.8 && newActualZoomLevel < 10;
            return {
                zoomLevel: {
    ...
    render() {
        ...
        return (
            <div className="orbits">
                ...
                {
                    popup.isActive &&
                    <Popup data={popup.data} handleClose={() => this.handleClose()} />
                }
            </div>
        );
    }
...
export default translate('authorization')(OrbitsCanvas);
